...
if Chef::Config[:solo] && !chef_solo_search_installed?
  ...
  search('users', node['openvpn']['user_query']) do |u|
    ...
    %w(conf ovpn).each do |ext|
      template "#{node['openvpn']['key_dir']}/#{node['openvpn']['config_name']}-#{u['id']}.#{ext}" do
    ...
    execute "create-openvpn-tar-#{u['id']}" do
      ...
      command <<-EOH
        tar zcf #{u['id']}.tar.gz ca.crt #{u['id']}.crt #{u['id']}.key #{node['openvpn']['config_name']}-#{u['id']}.conf #{node['openvpn']['config_name']}-#{u['id']}.ovpn
      EOH
